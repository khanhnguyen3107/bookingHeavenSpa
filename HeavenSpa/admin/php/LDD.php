...
$sql = "SELECT COUNT(*) AS total_rows 
        ...
        WHERE (MaKH IN (SELECT MaKH FROM khachhang WHERE TenKH LIKE '%$search%') 
            ...
            AND datlich.TrangThai = 'Đã duyệt'";
...
if (isset($_POST["Duyet"])) {
    ...
    $update_sql = "UPDATE datlich SET TrangThai = 'Đã duyệt' WHERE MaDL = '$maDL'";
    $update_result = $conn->query($update_sql);
    if ($update_result) {
        echo "<script>alert('Đã duyệt trạng thái'); window.location.href='QLLD.php';</script>";
        exit();
    } else {
        echo "<script>alert('Lỗi duyệt trạng thái'); window.location.href='QLLD.php';</script>";
        exit();
    }
}
?>
                ...
                <div class="col-lg-12">
                    <div class="ibox float-e-margins">
                        <div class="ibox-title">
                            <h5>Danh Sách Lịch Đã Duyệt Của Heaven Spa</h5>
                        </div>
                            ...
                            <table class="table table-striped table-bordered table-hover dataTables-example">
                                ...
                                <tbody>
                                    ...
                                    $sql = "SELECT datlich.MaDL, datlich.MaDV, datlich.MaKH, datlich.ThoiGianDat, datlich.NgayDat, datlich.TrangThai, datlich.ThoiLuong
                                        ...
                                        WHERE (khachhang.TenKH LIKE '%$search%' OR dichvu.TenDV LIKE '%$search%') AND datlich.TrangThai = 'Đã duyệt'
                                    ...
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            echo "<td><input type='submit' name='Duyet' value='Hủy'></td>";
                                            echo "</form>";
                                        }
                                    }
